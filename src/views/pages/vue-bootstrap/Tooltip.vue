  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Tooltips</b> Easily add tooltips to elements or components via the &lt;b-tooltip&gt; component or v-b-tooltip directive (preferred method).
        <a class="font-weight-bold" href="https://bootstrap-vue.js.org/docs/components/tooltip" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
    ...
    <div class="row">
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Tooltips'">
          <template v-slot:preview>
            <div class="text-center my-3">
              <b-button class="mr-3" v-b-tooltip.hover title="Tooltip directive content">
                Hover Me
              </b-button>
              <b-button class="mr-3" id="tooltip-target-1">
                Hover Me
              </b-button>
              <b-tooltip class="mr-3" target="tooltip-target-1" triggers="hover"> I am tooltip <b>component</b> content! </b-tooltip>
            </div>
          </template>
        ...
        <KTCodePreview v-bind:title="'Disabled elements'">
          <template v-slot:preview>
            <p>
              Elements with the <code>disabled</code> attribute aren't interactive, meaning users cannot focus, hover, or click them to trigger a
              tooltip (or popover). As a workaround, you'll want to trigger the tooltip from a wrapper <code>&lt;div&gt;</code> or
        ...
        <KTCodePreview>
          <template v-slot:title> <code>&lt;b-tooltip&gt;</code> component usage </template>
          <template v-slot:preview>
            <b-container fluid>
              <b-row>
      ...
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Programmatically show and hide tooltip'">
          <template v-slot:preview>
            <p>
              You can manually control the visibility of a tooltip via the syncable Boolean <code>show</code> prop. Setting it to
              <code>true</code> will show the tooltip, while setting it to <code>false</code> will hide the tooltip.
            </p>
            <div class="text-center">
              ...
              <b-tooltip :show.sync="show" target="tooltip-button-1" placement="top"> Hello <strong>World!</strong> </b-tooltip>
            </div>
          </template>
          <template v-slot:html>
            {{ code4.html }}
        ...
        <KTCodePreview v-bind:title="'Programmatic control'">
          <template v-slot:preview>
            <p>
              Programmatic control can also be affected by submitting '<code>open</code>' and '<code>close</code>' events to the tooltip by reference.
            </p>
            <div class="d-flex flex-column text-md-center">
              ...
              <b-tooltip ref="tooltip" target="tooltip-button-show-event"> Hello <strong>World!</strong> </b-tooltip>
            </div>
          </template>
          <template v-slot:html>
            {{ code5.html }}
        ...
        <KTCodePreview>
          <template v-slot:title> <code>v-b-tooltip</code> directive usage </template>
          <template v-slot:preview>
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vue Bootstrap', route: 'alert' }, { title: 'Notify', route: 'popover' }, { title: 'Tooltip' }]);
  }
};
</script>
