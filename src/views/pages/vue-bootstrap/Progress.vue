  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Progress</b> Use our custom progress component for displaying simple or complex progress bars, featuring support for horizontally stacked
        bars, animated backgrounds, and text labels.
        <a class="font-weight-bold" href="https://bootstrap-vue.js.org/docs/components/progress" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
      ...
      <div class="col-md-12">
        <KTCodePreview v-bind:title="'Progress'">
          <template v-slot:preview>
            <div>
              <b-progress :value="code1.value" :max="code1.max" show-progress animated></b-progress>
              <b-progress class="mt-2" :max="code1.max" show-value>
                <b-progress-bar :value="code1.value * (6 / 10)" variant="success"></b-progress-bar>
                <b-progress-bar :value="code1.value * (2.5 / 10)" variant="warning"></b-progress-bar>
                <b-progress-bar :value="code1.value * (1.5 / 10)" variant="danger"></b-progress-bar>
              </b-progress>
        ...
        <KTCodePreview v-bind:title="'Labels'">
          <template v-slot:preview>
            <p>
              ...
              <code>show-progress</code> (percentage of max) or <code>show-value</code> for the current absolute value. You may also set the precision
              (number of digits after the decimal) via the <code>precision</code> prop (default is 0digits after the decimal).
            </p>
            <div>
              <h5>No label</h5>
              <b-progress :value="code2.value" :max="code2.max" class="mb-3"></b-progress>

              <h5>Value label</h5>
              <b-progress :value="code2.value" :max="code2.max" show-value class="mb-3"></b-progress>

              <h5>Progress label</h5>
              <b-progress :value="code2.value" :max="code2.max" show-progress class="mb-3"></b-progress>

              <h5>Value label with precision</h5>
              <b-progress :value="code2.value" :max="code2.max" :precision="2" show-value class="mb-3"></b-progress>

              <h5>Progress label with precision</h5>
              <b-progress :value="code2.value" :max="code2.max" :precision="2" show-progress class="mb-3"></b-progress>
            </div>
          </template>
          <template v-slot:html>
            {{ code2.html }}
        ...
        <KTCodePreview v-bind:title="'Custom progress label'">
          <template v-slot:preview>
            <p>
              ...
              <code>&lt;b-progress-bar&gt; sub-component, or by using the label or label-html property on &lt;b-progress-bar&gt;</code>:
            </p>
            <div>
              ...
              <b-progress :max="code3.max" height="2rem">
                <b-progress-bar :value="code3.value">
                  Progress:
                  <strong>{{ code3.value.toFixed(2) }} / {{ code3.max }}</strong>
                </b-progress-bar>
              </b-progress>

              <h5 class="mt-3">Custom label via property</h5>
              <b-progress :max="code3.max">
                <b-progress-bar :value="code3.value" :label="`${((code3.value / code3.max) * 100).toFixed(2)}%`"></b-progress-bar>
              </b-progress>

              <h5 class="mt-3">Custom label via property (HTML support)</h5>
              <b-progress :max="code3.max">
                <b-progress-bar :value="code3.value" :label-html="`<del>${code3.value}</del>`"></b-progress-bar>
              </b-progress>
            </div>
          </template>
          <template v-slot:html>
        ...
        <KTCodePreview v-bind:title="'Solid background variants'">
          <template v-slot:preview>
            <div>
              <div v-for="(bar, i) in code5.bars" class="row mb-1" v-bind:key="i">
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vue Bootstrap', route: 'alert' }, { title: 'Progress' }]);
