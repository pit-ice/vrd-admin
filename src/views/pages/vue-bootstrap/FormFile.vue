  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Form File Input</b> Customized, cross-browser consistent, file input control that supports single file, multiple files, and directory
        upload (for browsers that support directory mode)
        <a class="font-weight-bold" href="https://bootstrap-vue.js.org/docs/components/form-file" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
    ...
    <div class="row">
        ...
        <KTCodePreview v-bind:title="'File name formatting via scoped slot'">
          <template v-slot:preview>
            <p>
              ...
              <code>file-name</code> to render the file names. The scoped slot will receive the following properties:
            </p>
      ...
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Control sizing'">
          <template v-slot:preview>
            <p>
              Use the <code>size</code> prop to control the visual size of the input. The default size is considered <code>md</code> (medium).
              Optional sizes are <code>lg</code> (large) and <code>sm</code> (small). These sizes line up with the sizes available on other form
              controls.
            </p>
            <div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vue Bootstrap', route: 'alert' }, { title: 'Form File' }]);
  }
};
</script>
