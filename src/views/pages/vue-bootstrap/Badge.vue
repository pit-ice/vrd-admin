  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Badges</b> Small and adaptive tag for adding context to just about any content.
        <a class="font-weight-bold" href="https://bootstrap-vue.js.org/docs/components/badge" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <KTCodePreview v-bind:title="'Actionable badges'">
          <template v-slot:preview>
            <p>
              Quickly provide actionable badges with hover and focus states by specifying either the <code>href</code> prop (links) or
      ...
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Contextual variations'">
          <template v-slot:preview>
            <p>
              Add any of the following <code>variants</code> via the variant prop to change the appearance of a <code>&lt;b-badge&gt;</code>:
              <code>default</code>, <code>primary</code>, <code>success</code>, <code>warning</code>, <code>info</code>, and <code>danger</code>. If
        ...
        <KTCodePreview v-bind:title="'Pill badges'">
          <template v-slot:preview>
            <p>
              Use the <code>pill</code> prop to make badges more rounded (with a larger border-radius and additional horizontal padding). Useful if
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vue Bootstrap', route: 'alert' }, { title: 'Badges' }]);
  }
};
</script>
