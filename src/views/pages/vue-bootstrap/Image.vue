  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Images</b> Documentation and examples for opting images (via &lt;b-img&gt; component) into responsive behavior (so they never become larger
        than their parent elements), optionally adding lightweight styles to them &mdash; all via props.
    ...
    <div class="row">
      <div class="col-md-12">
        <KTCodePreview v-bind:title="'Responsive images'">
          <template v-slot:preview>
            <p>
              ...
              <code>fluid</code> prop (which sets <code>max-width: 100%; height: auto;</code> via CSS classes) so that it scales with the parent
              element - up to the maximum native width of the image.
            </p>
            <div>
        ...
        <KTCodePreview v-bind:title="'Fluid grow'">
          <template v-slot:preview>
            <p>
              To make a fluid image that will grow to fill the width of its container, use the <code>fluid-grow</code> prop. Note this may cause
        ...
        <KTCodePreview v-bind:title="'Aligning images'">
          <template v-slot:preview>
            <p>
              Align images with the boolean props <code>left</code> (floats left) <code>right</code> (floats right), and <code>center</code> (auto
              left+right margins). You can also center images by placing them in a container that has the class <code>text-center</code>.
            </p>
            <div class="clearfix">
        ...
        <KTCodePreview v-bind:title="'Lazy loaded images'">
          <template v-slot:preview>
            <p>
              Use our complementary <code>&lt;b-img-lazy&gt;</code> image component (based on <code> &lt;b-img&gt;</code>) to lazy load images as they
              are scrolled into view (or within <code>offset</code> pixels of the viewport).
            </p>
            <div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vue Bootstrap', route: 'alert' }, { title: 'Images' }]);
  }
};
</script>
