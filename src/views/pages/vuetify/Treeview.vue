  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Treeview</b> The <code>v-treeview</code> component is useful for displaying large amounts of nested data.
        <a class="font-weight-bold" href="https://vuetifyjs.com/en/components/treeview" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
          ...
          <template v-slot:preview>
            <v-card class="mx-auto" max-width="500">
              <v-sheet class="pa-4 primary lighten-2">
                ...
                ></v-text-field>
                <v-checkbox v-model="code5.caseSensitive" dark hide-details label="Case sensitive search"></v-checkbox>
              </v-sheet>
              <v-card-text>
                <v-treeview :items="code5.items" :search="code5.search" :filter="code5.filter" :open.sync="code5.open">
          ...
          <template v-slot:preview>
            ...
            <v-card>
              ...
              <v-row>
                ...
                <v-col cols="12" md="6">
                  <v-card-text>
                    <div v-if="code7.tree.length === 0" key="title" class="title font-weight-light grey--text pa-4 text-center">
                      Select your favorite breweries
                    </div>

                    <v-scroll-x-transition group hide-on-leave>
                      <v-chip v-for="(selection, i) in code7.tree" :key="i" color="grey" dark small class="ma-1">
      ...
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Checkbox color'">
          <template v-slot:preview>
            ...
            <v-treeview selectable selected-color="red" :items="code2.items"></v-treeview>
          </template>
          <template v-slot:html>
            {{ code2.html }}
          </template>
        ...
        <KTCodePreview v-bind:title="'Slots'">
          <template v-slot:preview>
            <p>
              Using slots we are able to create an intuitive file explorer. Apart from the <code>prepend</code> slot, there is also one for the
              <code>label</code>, and an <code>append</code> slot.
            </p>
            <v-treeview v-model="code4.tree" :open="code4.open" :items="code4.items" activatable item-key="name" open-on-click>
                    ...
                    <v-card v-else :key="selected.id" class="pt-6 mx-auto" flat max-width="400">
                      <v-card-text>
                        <v-avatar v-if="code6.avatar" size="88">
                          <v-img :src="`https://avataaars.io/${code6.avatar}`" class="mb-6"></v-img>
                        </v-avatar>
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vuetify', route: 'alerts' }, { title: 'Treeview' }]);
  },

  computed: {
    filter() {
      return this.code5.caseSensitive ? (item, search, textKey) => item[textKey].indexOf(search) > -1 : undefined;
    },
