  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Chips</b> The <code>v-chip</code> component is used to convey small pieces of information. Using the <code>close</code> property, the chip
        becomes interactive, allowing user interaction.
        <a class="font-weight-bold" href="https://vuetifyjs.com/en/components/chips" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
            ...
            <v-card max-width="500">
              ...
              <v-container class="py-0">
                <v-row align="center" justify="start">
                  <v-col v-for="(selection, i) in code5.selections" :key="selection.text" class="shrink">
                  ...
                  <v-col v-if="!allSelected" cols="12">
                    <v-text-field ref="search" v-model="code5.search" full-width hide-details label="Search" single-line></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
              ...
              <v-list>
                <template v-for="(item, i) in categories">
                  <v-list-item v-if="!selected.includes(i)" :key="i" :disabled="code5.loading" @click="selected.push(i)">
                    <v-list-item-avatar>
                      <v-icon :disabled="code5.loading" v-text="item.icon"></v-icon>
                    </v-list-item-avatar>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :disabled="!selected.length" :loading="code5.loading" color="purple" text @click="next">Next</v-btn>
              </v-card-actions>
            </v-card>
          </template>
          <template v-slot:html>
          ...
          <template v-slot:preview>
            <v-card max-width="400">
              ...
              <v-row class="px-6 py-3" align="center">
                ...
                <v-menu v-model="code6.menu" bottom right transition="scale-transition" origin="top left">
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: 'Vuetify', route: 'alerts' }, { title: 'Chips' }]);
  },
