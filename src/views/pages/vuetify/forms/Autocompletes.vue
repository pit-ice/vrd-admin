  ...
  <div>
    <b-alert show variant="light" class="alert alert-custom alert-white alert-shadow fade show gutter-b">
      ...
      <div class="alert-text">
        <b>Autocompletes</b> The <code>v-autocomplete</code> component offers simple and flexible type-ahead functionality. This is useful when
        searching large sets of data or even dynamically requesting information from an API.
        <a class="font-weight-bold" href="https://vuetifyjs.com/en/components/autocompletes" target="_blank">
          See documentation.
        </a>
      </div>
    </b-alert>
      ...
      <div class="col-md-6">
        <KTCodePreview v-bind:title="'Custom filter on autocomplete'">
          <template v-slot:preview>
            <p>The <code>filter</code> prop can be used to filter each individual item with custom logic. In this example we filter items by name</p>
            <v-card class="overflow-hidden" color="purple lighten-1" dark>
              <v-toolbar flat color="purple">
                ...
                <v-spacer></v-spacer>
                <v-btn color="purple darken-3" fab small @click="code1.isEditing = !code1.isEditing">
                  <v-icon v-if="code1.isEditing">mdi-close</v-icon>
                  <v-icon v-else>mdi-pencil</v-icon>
                </v-btn>
              </v-toolbar>
              <v-card-text>
                <v-text-field :disabled="!code1.isEditing" color="white" label="Name"></v-text-field>
                <v-autocomplete
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :disabled="!code1.isEditing" color="success" @click="save">
                  Save
                </v-btn>
              </v-card-actions>
              <v-snackbar v-model="code1.hasSaved" :timeout="2000" absolute bottom left>
        ...
        <KTCodePreview v-bind:title="'Advanced slots'">
          <template v-slot:preview>
            <p>
              The <code>v-autocomplete</code> component is extremely flexible and can fit in just about any use-case. Create custom displays for
              <strong>no-data</strong>, <strong>item</strong> and <strong>selection</strong> slots to provide a unique user experience. Using
              <em>slots</em> enables you to easily customize the desired look for your application.
            </p>
            <v-toolbar color="orange accent-1">
              ...
              <template v-slot:extension>
                <v-tabs v-model="code3.tab" :hide-slider="!model" color="blue-grey" slider-color="blue-grey">
            ...
            <v-card color="blue-grey darken-1" dark :loading="isUpdating">
                ...
                <v-row>
                  ...
                  <v-row class="pa-4" align="center" justify="center">
                    <v-col class="text-center">
                      ...
                      <span class="grey--text text--lighten-1">{{ code2.title }}</span>
                    </v-col>
                  </v-row>
                </v-row>
              </v-img>
              <v-form>
                <v-container>
                  <v-row>
                    <v-col cols="12" md="6">
                      <v-text-field v-model="code2.name" :disabled="isUpdating" filled color="blue-grey lighten-2" label="Name"></v-text-field>
                    </v-col>
                    <v-col cols="12" md="6">
                      <v-text-field v-model="code2.title" :disabled="isUpdating" filled color="blue-grey lighten-2" label="Title"></v-text-field>
                    </v-col>
                    <v-col cols="12">
                      <v-autocomplete
              ...
              <v-card-actions>
                ...
                ></v-switch>
                <v-spacer></v-spacer>
                <v-btn :disabled="code2.autoUpdate" :loading="isUpdating" color="blue-grey darken-3" depressed @click="isUpdating = true">
          ...
          <template v-slot:preview>
            <v-card>
              ...
              <v-card-text>
                ...
                <v-autocomplete
                  v-model="code4.model"
                  :hint="!code4.isEditing ? 'Click the icon to edit' : 'Click the icon to save'"
                  :items="code4.states"
                  :readonly="!code4.isEditing"
                  :label="`State — ${code4.isEditing ? 'Editable' : 'Readonly'}`"
                  persistent-hint
                  ...
                  <template v-slot:append-outer>
                    <v-slide-x-reverse-transition mode="out-in">
                      <v-icon
                        ...
                        v-text="code4.isEditing ? 'mdi-check-outline' : 'mdi-circle-edit-outline'"
                      ></v-icon>
